...
void i2c_Init(){
	...
	LPC_IOCON->PIO0_4 |= (1 << 1);			// P0.4 set to SCL (pin 27)
	LPC_IOCON->PIO0_5 |= (1 << 1);			// P0.5 set to SDA (pin 5)
...
void i2c_begin(char addr, int rw){
	...
	STASET;
	SICLR;
	printf("interrupt flag before wait: %x\n\r", I2C_SI);
	// wait for interrupt
	while(!(0x8 && lpc_i2c->CONSET)){
		//printf("interrupt flag while wait: %x\n\r", I2C_SI);
	}
	// for debugging
	printf("interrupt flag after wait: %x\n\r", I2C_SI);
	printf("stat reg after wait: %x\n\r", I2C_STAT_REG);
		
	// wait until succeed
	while(I2C_STAT_REG != I2C_EVENT_START){
		SICLR;
		while(!(0x8 && lpc_i2c->CONSET)){};
		// for debugging
			//printf("stat reg: %x\n\n", I2C_STAT_REG);
